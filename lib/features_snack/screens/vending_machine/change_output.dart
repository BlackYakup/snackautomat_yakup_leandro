...
class ChangeOutput extends ConsumerStatefulWidget {
  const ChangeOutput({
    super.key,
  ...
  @override
  ConsumerState<ChangeOutput> createState() => _ChangeOutputState();
}

class _ChangeOutputState extends ConsumerState<ChangeOutput>
  ...
  void initState() {
    ...
    // ExpansionTile baut dieses Widget oft erst, wenn schon Wechselgeld da ist.
    // Dann lief bisher keine Animation → Opacity 0 (Münzen unsichtbar).
    if (widget.session.hasPendingChange) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !widget.session.hasPendingChange) return;
  ...
  void didUpdateWidget(covariant ChangeOutput oldWidget) {
    ...
    // Gleiche Signatur, aber Animation nie gestartet (z. B. nach Aufklappen).
    if (widget.session.hasPendingChange &&
        _controller.value == 0 &&
  ...
  void _onTap() {
    ...
    ref.read(vendingSessionProvider.notifier).collectOutputChange();
  }
  ...
  Widget _buildFallingCoin({
    ...
    // Solange die Drop-Animation noch bei 0 hängt: Münze trotzdem voll zeigen.
    final raw = _controller.value == 0 && !_controller.isAnimating
        ? 1.0
        : ((_controller.value - start) / (end - start)).clamp(0.0, 1.0).toDouble();
    final progress = Curves.bounceOut.transform(raw);
