...
│
├── assets/presentation/diagrams/ + brand/ + status/
...
| Status | `assets/presentation/status/` | Status-Grafiken für Folien |
...
Exakter Pfad hängt vom Package-Namen und der Plattform ab — zur Laufzeit in der Debug-Ausgabe von `Power3dBootstrap` sichtbar.
...
Debug-Lauf (aktuell oft unter `.dart_tool`):

```text
SQLite-Datenbank: …\snackautomat.db
```
...
## 9. Dev-Pipeline-Pfade (Modellpflege außerhalb)

| Artefakt | Hinweis |
| --- | --- |
| Master-Blend | nicht im Repo — lokal pflegen |
| Export-Skript | historisch `v5_export_flutter_front_glb.py` |
| Shading-Fix | historisch `v5_fix_shading_normals.py` |
| Ziel-GLB (App) | `assets/models/vending_machine_front.glb` |
| Roh-Assets | Keypad / ePort / CoinMod / Textures (extern) |
...
| Was | Pfad |
| --- | --- |
| Markdown-Doku | `assets/install/*.md` |
