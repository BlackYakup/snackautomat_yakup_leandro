...
**Stand:** August 2026 · Export-GLB `assets/models/vending_machine_front.glb` (~85 MB)

**Quellen:** `lib/main.dart`, `lib/features_snack/providers/`, `lib/features_snack/screens/vending_machine/vending_machine_3d_landing_screen.dart`, `packages/power3d/lib/src/controller/projection_extension.dart`.
...
flowchart LR
  ...
  Blend[Blender_offline] -->|Export| GLB
...
**Kernaussage:** Die Demo läuft standalone (GLB + SQLite). Live-Blender nur für Modellarbeit/Export außerhalb des Repos.
...
| `ProductSlot` | `models/product/product_slot.dart` | Belegung: `productId`, Reihe, Spalte, `stockQuantity` → Slot-Code z. B. `A1` |
...
| E–F | 1–5 (UI) Dual-Breite | F1…F5 | Physisch Spalten 1,3,5,7,9; beide Spiralen `start` + `start+1` |
...
Quelle: enum `VendingMachinePhase` in `providers/`.
...
Landing `_syncHudOled` mappt `VendingMachinePhase` → `updateVendingHudOled` auf Mesh `*HUD_Screen*` (DynamicTexture). Parallel: 2D-`DigitalDisplay` (SNACK-OLED).
