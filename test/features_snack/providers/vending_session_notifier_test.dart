...
Future<_TestHarness> _createHarness({
  ...
  await container.read(placedProductsProvider.future);
...
void main() {
  ...
  group('Slot-Timeout und Leeren', () {
    test('ein einzelner Buchstabe wird nach drei Sekunden verworfen', () async {
      final harness = await _createHarness();
      addTearDown(harness.dispose);
    ...
    test('A1 wird nach drei Sekunden geprüft — leer zurück zur Auswahl', () async {
      final harness = await _createHarness();
      addTearDown(harness.dispose);

      fakeAsync((async) {
        // C4 ist belegt im Testgerüst; A1 typischerweise leer.
        harness.notifier.pressSlotKey('A');
        harness.notifier.pressSlotKey('1');
        expect(harness.state.currentSlotInput, 'A1');
        ...
        expect(harness.state.selectedProduct, isNull);
        expect(harness.state.phase, VendingMachinePhase.ready);
        expect(harness.state.statusMessage.toLowerCase(), contains('leer'));
      });
    });

    test('A1 + weitere Taste vor Timeout ermöglicht A10', () async {
      final harness = await _createHarness();
      addTearDown(harness.dispose);
      ...
      fakeAsync((async) {
        ...
        harness.notifier.pressSlotKey('0');
        expect(harness.state.currentSlotInput, 'A10');

        async.elapse(const Duration(seconds: 3));

        // A10 leer → Ruhezustand mit Meldung, kein Hängenbleiben auf Eingabe.
        expect(harness.state.currentSlotInput, isEmpty);
        expect(harness.state.phase, VendingMachinePhase.ready);
      });
    });
  ...
  group('Zahlung vor Ausgabe', () {
    test('Slot-Auswahl startet Zahlung ohne Bestandsänderung', () async {
      final harness = await _createHarness(stockQuantity: 3, priceCents: 160);
      addTearDown(harness.dispose);

      harness.notifier.selectProductBySlot('C4');
      ...
      expect(rows.single['stock_quantity'], 3);
      expect(await harness.database.query('transactions'), isEmpty);
    });

    test('Abbrechen mit X gibt Geld zurück und stoppt Kauf', () async {
      final harness = await _createHarness(priceCents: 160);
      addTearDown(harness.dispose);
      ...
      expect(harness.state.outputChange, {100: 1});
      expect(harness.state.outputProduct, isNull);
      expect(harness.state.canUseSlotKeys, isFalse);

      harness.notifier.collectOutputChange();

      expect(harness.state.outputChange, isEmpty);
      expect(harness.state.hasPendingChange, isFalse);
      expect(harness.state.canUseSlotKeys, isTrue);
      expect(
        harness.state.statusMessage,
        contains('Wechselgeld entnommen'),
      );
    });

    test('nach vollständiger Zahlung Bestand −1 und Münzen in Kassette', () async {
      final harness = await _createHarness(stockQuantity: 3, priceCents: 160);
      addTearDown(harness.dispose);
      final before100 = harness.state.coinInventory[100]!;
      ...
      await _waitUntil(
        () =>
            harness.state.phase == VendingMachinePhase.thankYou ||
            harness.state.outputProduct != null,
      );

      final placed = harness.container.read(placedProductsProvider).value!;
      expect(
        placed.where((p) => p.slotCode == 'C4').single.stockQuantity,
        2,
      );
      ...
      expect(rows.single['stock_quantity'], 2);
      expect(harness.state.coinInventory[100], before100 + 1);

      final transactions = await harness.database.query('transactions');
      expect(transactions, hasLength(1));
      expect(transactions.single['amount_paid_cents'], 160);
      expect(transactions.single['product_id'], harness.product.id);
    });

    test('ausverkauft wird abgelehnt ohne Bestand zu ändern', () async {
      final harness = await _createHarness(stockQuantity: 0);
      addTearDown(harness.dispose);
    ...
    test('leerer Slot wird abgelehnt', () async {
      final harness = await _createHarness();
      addTearDown(harness.dispose);

      harness.notifier.selectProductBySlot('A1');
      ...
      expect(
        harness.state.statusMessage.toLowerCase(),
        anyOf(contains('leer'), contains('kein produkt'), contains('ungültig')),
      );
    });
  });
}
