...
**Stand:** August 2026 · App `1.0.0+1` · SDK `^3.11.0` · Master-Blend **`nur noch exportiertes GLB im Repo`**
...
| Blender (Dev) | **bpy** / **bmesh** in Blender | Modellbau + GLB-Export unter `entferntes Blender-Archiv` |
| Agent-Dev | Blender MCP `:9877` | Nur Szene — kein App-Kauf-Backend |
...
| **`power3d`** | **2.3.1** | path: `packages/power3d` | Babylon-Viewer, Dispense-API | `vending_machine_3d_landing_screen.dart`, `packages/power3d/lib/**` |
| `flutter_riverpod` | 3.3.2 | ^3.3.2 | Session-State, Repos | `providers/provider.dart`, Screens |
...
| Paket | Version (lock) | Constraint | Wofür | Wo genutzt |
| --- | --- | --- | --- | --- |
| `flutter_test` | SDK | sdk | Tests | `test/**` |
...
| `fake_async` | 1.3.3 | ^1.3.3 | Zeit-Mocks in Tests | `test/**` |
...
Runner unter `windows/` aktiv; andere Plattform-Runner archiviert unter `entferntes Blender-Archiv`. **Produktionsfokus** der 3D-Demo: Windows.

---

## 7. Blender / Python (Modell-Toolchain, Dev only)

Kein `pip install` für die **App-Laufzeit**. Skripte laufen **in Blender** unter `entferntes Blender-Archiv`:

| Modul | Zweck |
| --- | --- |
| `bpy` / `bmesh` | Geometrie, Materialien, Export |
| `mathutils` | Matrizen / Vektoren |
| PolyHaven | Addon unter `ehem. PolyHaven` |

| Datei | Rolle |
| --- | --- |
| **`nur noch exportiertes GLB im Repo`** | Master-Szene |
| `ehem. Blender-Skripte/v5_export_flutter_front_glb.py` | Export → Flutter-GLB |
| `ehem. Blender-Skripte/v5_fix_shading_artifacts.py` | Shading-Fixes |
| MCP `:9877` | Nur Dev/Agent |
...
| Tool | Zweck | Pfad |
| --- | --- | --- |
| Python 3 | PDF-Generierung | `install/generate_pdf.py` |
| reportlab | PDF-Fallback | wird bei Bedarf per pip installiert |
| pandoc | optionaler PDF-Weg | falls installiert |
| Node.js | Doku-Regen, Next.js | `install/regen/`, `page/` |
| Cursor SDK | Doku-Regeneration | `install/regen/src/regen-docs.ts` |
...
4. Optional: `nur noch exportiertes GLB im Repo` öffnen → bei Änderungen `v5_export_flutter_front_glb.py`
...
| Schwarzes / falsches Shading | Style + `v5_fix_shading_artifacts.py` |
...
| PDF-Doku | `install/generate_pdf.py` (+ reportlab) |
| Doku-Regen | `install/regen/` via Cursor SDK |
